...
Since all our constraints and objective functions are quadratic, one can show that both problems above can be written in a way that Theorem~\ref{thm:bienstock} can be applied. Of course, this only gives us an approximate solution. However, one can choose the $\varepsilon$ as an appropriate function of $T$ such that the approximate solution is still good enough that it does not increase the regret. Since the runtime depends logarithmically on $1/\varepsilon$, this only costs us an extra $\log T$ factor in the runtime. Details are in Appendix~\ref{app:hard-computation}.
...
\draft{
\begin{proof}
The proof relies on the following lemma, which we state without proof. Its
precise version and proof are in Appendix~\ref{app:soft-square-root-analysis},
as Equation~\ref{eq:soft-subspace-certificate}.

\begin{lemma}\label{lem:sketch-soft-certificate}
With probability at least $1-\delta$, every matrix $V$ that
Algorithm~\ref{alg:soft-square-root} considers during the $T$ rounds satisfies
$\operatorname{dist}\left(w(x,y),\mathcal{L}\right)\le\tilde{O}\left(\sqrt{p_V(x,y)}\right)$
for all $x\in X$ and $y\in D$, where $\tilde{O}$ hides a factor polynomial in the
dimensions and logarithmic in $T$ and $\frac{1}{\delta}$.
\end{lemma}

This is the counterpart of Lemma~\ref{lem:sketch-ellipsoid-close}. There the bound
held for the points of the ellipsoid $E_V$; here it holds for every point, with
$\sqrt{p_V(x,y)}$ measuring how far outside the ellipsoid the point lies.

On the event of Lemma~\ref{lem:sketch-soft-certificate},
Lemma~\ref{lem:sketch-close-reward} gives, for every arm $x'$ and
outcome $y\in D$,
...
$U_{V,\lambda}(x')\ge v^\star(x')-\tilde{O}(1/\lambda)$.
Since $x$ maximizes the score,
$U_{V,\lambda}(x)\ge V^\star-\tilde{O}(1/\lambda)$.
Finally, the definition of the score gives
$U_{V,\lambda}(x)\le r(x,y)+\lambda p_V(x,y)$ for every $y\in D$.
...
when maximizing $U_{V,\lambda}$. This adds at most $n\varepsilon$ to the regret
...
\begin{equation*}
v^{\star} \left( x \right) \le r \left( x , y \right) + G B_{T}^{\prime} \sqrt{p_{V} \left( x , y \right)} .
\end{equation*}

...
\draft{
We use the following consequence of Bienstock's theorem~\citep{bienstock2016cdt} for both problems.
...
The theorem allows small constraint violations, so its output does not
...
Bienstock's theorem as described below. After rounding the output to
...
We maximize this expression jointly over $x\in X$ and $z$ to additive
accuracy $\varepsilon$ to choose the next arm.

Both maximizations reduce to quadratic programs with a fixed number
of quadratic constraints. For planning, replace the two norms in the
...
quadratic in $\left( x , z , t , s \right)$. For the distance problem, use
$\|z_D\|_2^2\le s^2$ instead. All variables have explicit bounds, so we
can add a redundant enclosing ball and apply Bienstock's theorem.
Projecting and rounding the returned points into the $x$ and $z$ balls,
and rounding the norm values upward, gives valid rational arms and
certified lower bounds. The precision needed for these steps, the
distance and planning formulas, and the bound on $M$ are justified in
Appendix~\ref{app:hard-computation}.

The regret argument still works. An arm chosen by the emptiness test
forces an update even after rounding,

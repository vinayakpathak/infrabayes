...
Making sure that an artificial superintelligence does not pursue goals that are unaligned with humans is a challenging problem, and arguably one of the most important of our times~\citep{ngo2024alignment,bengio2024managing}. Several attempts have been made to get a handle on it~\citep{russell2015research,amodei2016concrete,everitt2018agi,ji2023alignment} and a recurring theme is that at its core it is a learning problem~\citep{dewey2011learning,bostrom2014superintelligence,soares2016value,russell2019human,ng2000algorithms,hadfieldmenell2016cooperative,hadfieldmenell2017inverse,milli2017obedient,jeon2020reward,christiano2017deep,leike2018scalable,ouyang2022training,bai2022training,shah2019preferences,armstrong2018occam,hubinger2019risks,casper2023open}. Indeed, imagine several agents coexisting in the universe and interacting with each other, with each agent trying to act in the interest of a subset of the agents. The agents only have partial information about each other's behaviours and goals as well as about the universe, and must ensure the universe evolves towards desirable states. Given the role of learning, it is not unreasonable to imagine that learning theory might help us pave the way towards solving the problem~\citep{kosoy2018learningagenda}.

However, the currently known results in learning theory are likely insufficient to solve alignment. For example, it has been suggested that designing better learners for unrealizable learning is crucial for solving the problem~\citep{kosoy2018learningagenda, demski2019embedded}. Realizability is a strong assumption in general, but it becomes even more questionable in the multiagent learning scenarios described above.

One concrete issue with realizable learning is the grain of truth problem~\citep{hutter2009open,leike2016grain,wyeth2025grain,kosoy2023agenda}. Suppose Alice and Bob are playing a known game. Neither knows the other's strategy but assumes that they lie in a class $H$ of strategies~\citep{kalai1993rational,noguchi2015bayesian}. Can each player simultaneously achieve a low regret? The issue is that the strategy that achieves a low regret against all strategies in $H$ may not itself be a member of $H$~\citep{nachbar1997prediction,nachbar2005beliefs}. In that case, if Alice's strategy is in $H$, then Bob's strategy will have to be outside of $H$ and at least one of the players needs to solve an unrealizable learning problem. Finding a general and natural class $H$ whose learner lies within $H$ is called the grain of truth problem that remains open despite some recent progress using \emph{reflective oracles}~\citep{fallenstein2015reflective,leike2016grain,wyeth2025grain}.

More generally, realizability works only when the agent is more expressive than its environment. However, if the environment contains other agents that are similar or more powerful than the agent doing the learning then realizability fails. This can happen, for example, when Alice and Bob are running similar source codes~\citep{tennenholtz2004program,barasz2014robust}, or when Bob is able to predict Alice's actions~\citep{critch2019bounded,oesterheld2019robust}. 

The usual response to unrealizability is to provide agnostic learning guarantees. Here, one makes no assumption about the environment but guarantees that the learner's performance is competitive among a given class of strategies~\citep{kearns1994agnostic,bendavid2009agnostic}. However, designing learners with agnostic learning guarantees has been difficult. In supervised learning, there are many natural examples where agnostic learning is computationally intractable even though the realizable version is easy~\citep{daniely2016halfspaces,daniely2016dnf}. In more sophisticated learning frameworks, agnostic learning is even statistically hard~\citep{krishnamurthy2016pac,sekhari2021agnostic}. In online learning, one can generally get a much better regret for the realizable case than for agnostic~\citep{littlestone1988learning,bendavid2009agnostic}. In reinforcement learning, the agnostic case is usually statistically intractable~\citep{li2025agnostic,jia2023agnostic,sekhari2021agnostic} and only very simple problems are known to have good sample complexity bounds.

Closer to our current work, in an ``agnostic'' version of the bandits problem, one makes no assumptions about the mapping from arms to reward, but competes against the best policy in a given class. This can work if the class contains a small number of policies~\citep{auer2002nonstochastic}, however, impossibility results are easy to prove for infinite policy classes~\citep{bubeck2011pure,bubeck2011xarmed}. Moreover, even for finite classes, since the regret grows as the square root of the size of the policy class, the situation is hopeless if the class is finite but large. 

\paragraph{Our results} In this paper, we build on the framework of imprecise bandits as introduced in~\citep{kosoy2025imprecise}, which can be seen as an alternative to agnostic learning in the unrealizable case. The general idea is to say that we still have a known hypothesis class that contains a ground truth hypothesis, however, each hypothesis in our class provides only a partial specification of the environment. Given such a hypothesis, an adversary can pick any environment that is consistent with the specifications. The task of the learner is to compete with the worst-case environment for the ground truth hypothesis. Details can be found in Section~\ref{sec:setting}. In~\citep{kosoy2025imprecise}, a class of instances of the imprecise bandits problem, which can be seen as a generalization of stochastic linear bandits, was shown to have a $\Theta(\sqrt{T})$ regret. Here, we investigate the computational complexity of the learner. We show that the setting of~\citep{kosoy2025imprecise} is computationally intractable in general and identify a special case that admits a polynomial time learner achieving $O(\sqrt{T})$ regret. We also show that generalizing this special instance in minor ways quickly leads to computational intractability, thus demonstrating that it is at the boundary of what is tractable.

\section{Setting}
\label{sec:setting}
A \emph{credal set} over a domain $D$ is a set of probability distributions over $D$ that is closed under convex combinations. We denote by $\credal D$ the set of all credal sets over $D$.

An instance of the imprecise bandits problem is a tuple $\mathcal{M} = \left( X , D , H , r \right)$, where $X$ is a set of arms, $D$ is a set of possible outcomes, $H$ is a set of mappings of type $X \to \credal D$, and $r : X \times D \to \mathbb{R}$ is a reward function.
...
{Write $C_{r} \coloneqq \operatorname*{max}_{X \times D} r - \operatorname*{min}_{X \times D} r$ for the range of the reward. Since $r$ is affine and $X$ and $D$ are balls, $C_{r} = 2 R_{X} {\left\Vert a \right\Vert}_{2} + 2 R_{D} {\left\Vert b \right\Vert}_{2}$.}
...
\draft{Here $B_{z}$, $C_{z}$, and $d_{z}$ are matrices and vectors of fixed
dimensions, whose entries are affine functions of $z$. Now, hypothesis
...
\end{equation*}
  ...
  and $C_{r}$. The precise dependence is given in
  Theorem~\ref{thm:soft-square-root-regret}.

\end{theorem}


This dependence on $T$ is tight up to logarithmic factors. We first observe that
linear imprecise bandits contain stochastic linear bandits as a special case, and then
transfer the known lower bound for stochastic linear bandits.

\begin{remark}\label{rem:stochastic-linear-bandits}
Linear imprecise bandits contain stochastic linear bandits as a special case. In a
stochastic linear bandit, the learner picks arms $x_{t}$ from the unit ball
$X \subseteq \mathbb{R}^{d}$ and observes a reward in $\left[ - 1 , 1 \right]$ whose mean is
$\theta^{T} x_{t}$, for an unknown $\theta$ in the unit ball of $\mathbb{R}^{d}$. To write
this as a linear imprecise bandit, we let the outcome be the reward itself:
take $D = \left[ - 1 , 1 \right]$, $r \left( x , y \right) = y$, let $Z$ be the unit ball of $\mathbb{R}^{d}$,
and set


\begin{equation*}
K_{\theta} \left( x \right) \coloneqq \left\{ y \in D : y - \theta^{T} x = 0 \right\} = \left\{ \theta^{T} x \right\} .
\end{equation*}


This has the required form with $C_{\theta} = 1$, $B_{\theta} = - \theta^{T}$, and
$d_{\theta} = 0$. Since $\left| \theta^{T} x \right| \le 1$, the only solution of the constraint
already lies in $D$, so transversality holds with $S = 1$. The hypothesis
$h_{\theta} \left( x \right)$ now contains every distribution on $\left[ - 1 , 1 \right]$ with mean $\theta^{T} x$.
In particular, it contains the reward distribution of the stochastic linear
bandit at $x$, and the adversary may choose that distribution at every round.
Against this adversary, the learner sees exactly what it would see in the
stochastic linear bandit. The regret is also the same in both problems, since
$v^{\ast} \left( x \right) = \theta^{T} x$ and $V^{\star} = \operatorname*{max}_{x \in X} \theta^{T} x$. So any learner for linear
imprecise bandits is also a learner for stochastic linear bandits with the
same regret.
\end{remark}
{
\begin{theorem}[lower bound, informal]\label{thm:lower-bound-informal}
Every learner for linear imprecise bandits has worst-case regret $\Omega \left( \sqrt{T} \right)$.
\end{theorem}

\begin{proof}
Stochastic linear bandits have a worst-case regret lower bound of
$\Omega \left( \sqrt{T} \right)$~\citep{dani2008stochastic}. By
Remark~\ref{rem:stochastic-linear-bandits}, the same bound holds for linear imprecise bandits.
\end{proof}
}

\section[Related work]{\draft{Related work}}\label{sec:related-work}

\draft{
\paragraph{Imprecise probabilities and credal sets} A hypothesis in our class assigns to each arm a credal set, a closed convex set of distributions, and our benchmark judges an arm by the worst distribution in that set. Both choices come from a long debate about how to represent an uncertain belief. The standard answer is a single probability distribution, which can be derived from axioms on rational preference~\citep{savage1954foundations, jaynes2003probability}. But a single distribution says more than the evidence often supports~\citep{ellsberg1961risk}. The natural repair is to keep every distribution that the evidence does not rule out. Levi called such a closed convex set of distributions a credal set~\citep{levi1974indeterminate,levi1980enterprise}, and Walley built a general theory of imprecise probability around these sets~\citep{walley1991statistical}. A credal set expresses the partial belief above directly. To act on it one needs a decision rule, and Gilboa and Schmeidler axiomatized the maximin rule, which evaluates an action under the worst distribution in the set~\citep{gilboa1989maxmin}. Our value $v_{h} \left( x \right)$ is this rule applied to $h \left( x \right)$. The one difference is what the set describes. In this literature a credal set is a belief, and evidence updates it. In our model $h \left( x \right)$ is a property of the environment: the adversary may pick any element of $h^{\star} \left( x \right)$ at every round, so no amount of data removes the uncertainty inside it. Credal sets play this role in robust Markov decision processes, where an adversary picks a transition kernel from a given set~\citep{iyengar2005robust,nilim2005robust}, and in infra-Bayesianism, which takes such sets as the hypotheses of a reinforcement learner~\citep{diffractor2020introduction}; the imprecise bandits problem~\citep{kosoy2025imprecise} is the bandit version of that proposal.

\paragraph{Partial specifications of the environment} Our hypotheses say only part of what there is to say about the environment. The closest analogue is the theory of partial concept classes of Alon, Hanneke, Holzman, and Moran~\citep{alon2021theory}. A partial concept is undefined on part of the domain, and a learner only sees examples where the true concept is defined, so a partial class expresses an assumption on the data, such as a margin condition, without committing to one total classifier. They show that this changes the theory: some partial classes are easy to learn even though every learner of them must use hypotheses of unbounded VC dimension, so the problem cannot be reduced to a realizable one by completing the concepts. Our hypotheses are partial at every arm rather than undefined at some points, and they also cannot be completed: every choice of one element of $h^{\star} \left( x \right)$ per arm is a stochastic bandit, but the adversary may change its choice every round. Pour, Mansouri, and Ben-David~\citep{pour2026learning} study online learning where each instance has a set of valid labels and the learner must output one of them. Their hypotheses also map inputs to sets, but there the set relaxes the learner's task, whereas ours relaxes what the hypothesis promises and the learner must be ready for every element.

\paragraph{Stochastic linear bandits and outcome feedback} Our linear instances generalize stochastic linear bandits~\citep{auer2002using,dani2008stochastic,rusmevichientong2010linearly,abbasi2011improved,chu2011contextual,lattimore2020bandit} in two ways. A hypothesis does not determine the mean outcome at an arm but only restricts it to the affine set $K_{z} \left( x \right)$, and the learner observes an outcome $y_{t}$ from which the reward is computed rather than the reward alone. When every $K_{z} \left( x \right)$ is a single point, the mean outcome is affine in the arm and we recover a linear bandit with vector-valued observations. Observing an outcome and computing the reward from it is the feedback structure of combinatorial semi-bandits~\citep{gyorgy2007online,chen2013combinatorial,kveton2015tight}, of partial monitoring~\citep{rustichini1999minimizing,cesabianchi2006regret,bartok2014partial,lattimore2019information}, whose linear version~\citep{kirschner2020information} is closest to our linear instances, and of the decision making with structured observations framework of Foster et al.~\citep{foster2021statistical}, which contains all of these. These models assume either that the true environment lies in the class or that the outcomes are arbitrary and the benchmark is the best fixed action. Ours sits between the two: the class contains a hypothesis that the environment satisfies, and the environment may change within it every round. Bandits with adversarial corruptions~\citep{lykouris2018stochastic,bogunovic2021stochastic} also let an adversary modify a stochastic environment, but there the adversary has a budget and the benchmark is the uncorrupted environment, whereas here the adversary is limited only by the credal set and the benchmark takes the worst case within it.

\paragraph{Smoothed analysis and smoothed adversaries} Our hypotheses restrict the adversary to a convex set of distributions at each arm. Smoothed online learning restricts the adversary in the same way, with a single known set. Smoothed analysis was introduced by Spielman and Teng to explain why the simplex method is fast in practice: the input is a worst-case instance perturbed by a small amount of random noise~\citep{spielman2004smoothed}. Rakhlin, Sridharan, and Tewari brought this idea to online learning~\citep{rakhlin2011online}, and Haghtalab, Roughgarden, and Shetty developed it into the smoothed adversary model: on every round the adversary picks a distribution over instances whose density relative to a fixed base measure is at most $1/\sigma$, and the instance is drawn from it~\citep{haghtalab2020smoothed,haghtalab2024smoothed}. The set of $\sigma$-smooth distributions is a credal set, so a smoothed adversary is an adversary of our kind, for one particular hypothesis. The main finding is that this restriction makes online learning as easy as statistical learning, with regret governed by the VC dimension rather than the Littlestone dimension~\citep{block2022smoothed}, and, closest to our question, that it admits learners that run in polynomial time given an offline optimization oracle for the class, where no such learner exists against a worst-case adversary~\citep{haghtalab2022oracle}. In contextual bandits, smooth contexts make the greedy algorithm nearly optimal, because the perturbations explore on their own~\citep{kannan2018smoothed,raghavan2023greedy}. Two things are different in our model. The credal set is not known: the learner must learn which set applies at each arm. And the benchmark is different: smoothed online learning competes with the best fixed predictor in hindsight, whereas we compete with the worst-case value of the true hypothesis, which is easier to reach. Taking the predictors as arms and the smooth set as the only hypothesis, a learner with low smoothed regret also has low regret in our sense, but not conversely.
}

{

\section{Warmup: A $T^{2/3}$ Learner}\label{sec:hard-ellipsoid-warmup}
...
determine a linear subspace of this vector space. Define $\mathcal{L} \coloneqq \operatorname{ker} \begin{pmatrix}d_{z^{\star}} & B_{z^{\star}} & C_{z^{\star}}\end{pmatrix} .$ A distribution is feasible for arm $x$ precisely when its expectation $m$ satisfies $m \in D$ and $w \left( x , m \right) \in \mathcal{L}$.
}{The learner's task is to learn enough about $\mathcal{L}$ to get low regret.
Of course, we may never learn all of $\mathcal{L}$. For example, if the adversary keeps
...
To choose an arm, we use our current guess for the feasible expected values. For each $x$, this consists of the $m \in D$ with
$\left( 1 , x , m \right) \in E_{V}$. If some arm has no such $m$, we choose it to learn
more about it. Otherwise, we use ``optimism in the face of uncertainty'' and choose the arm with the largest worst-case
...
\begin{algorithm2e}
\caption{}\label{alg:hard-ellipsoid-warmup}
\KwIn{Horizon $T$ and block length $n\in\{1,\ldots,T\}$.}
$V\leftarrow n^{-1}I_q$\;
\For{$j\leftarrow 1$ \KwTo $\lceil T/n\rceil$}{
  Define $\hat{K}_V(x)\coloneqq\{y\in D:p_V(x,y)\le1\}$ for every $x\in X$\;
  \eIf{there exists $x\in X$ with $\hat{K}_V(x)=\varnothing$}{
    Choose an arm $x\in X$ with $\hat{K}_V(x)=\varnothing$\;
  }{
    Define $H_V(x)\coloneqq\min_{y\in\hat{K}_V(x)}r(x,y)$ for every $x\in X$\;
    Choose $x\in\operatorname*{argmax}_{x'\in X}H_V(x')$\;
  }
  $n_j\leftarrow\min\{n,T-(j-1)n\}$\;
  \For{$i\leftarrow1$ \KwTo $n_j$}{
    Play $x$ and observe $y_i$\;
  }
  $\overline{y}\leftarrow\frac{1}{n_j}\sum_{i=1}^{n_j}y_i$,\quad
  $\overline{w}\leftarrow w(x,\overline{y})$\;
  \If{$n_j=n$ and $p_V(x,\overline{y})>1$}{
    $V\leftarrow V+\overline{w}\,\overline{w}^{T}$\;
  }
}
\end{algorithm2e}
\FloatBarrier
...
\begin{theorem}[Informal]\label{thm:hard-ellipsoid-warmup}
  Algorithm~\ref{alg:hard-ellipsoid-warmup} has expected regret $R_{T} \le \tilde{O} \left( n + \frac{T}{\sqrt{n}} \right)$. In particular, choosing $n = \left\lceil T^{2/3} \right\rceil$ gives
  $R_{T} \le \tilde{O} \left( T^{2/3} \right)$.
...
\begin{proof}
The sketch relies on three lemmas, which we state here without proof. Their
precise versions, with explicit constants, and their proofs are in
Appendix~\ref{app:hard-ellipsoid-analysis}: see, respectively, Lemma~\ref{lem:subspace-distance-bound},
Lemma~\ref{lem:subspace-reward-bound}, and the proof of Theorem~\ref{thm:hard-ellipsoid-regret}.

\begin{lemma}\label{lem:sketch-ellipsoid-close}
With probability at least $1 - \delta$, every ellipsoid $E_{V}$ that
Algorithm~\ref{alg:hard-ellipsoid-warmup} maintains during the $T$ rounds satisfies
$\operatorname{dist} \left( w , \mathcal{L} \right) \le \tilde{O} \left( \frac{1}{\sqrt{n}} \right)$ for all
$w \in E_{V}$, where $\tilde{O}$ hides a factor polynomial in the dimensions and
logarithmic in $T$ and $\frac{1}{\delta}$.
\end{lemma}

\begin{lemma}\label{lem:sketch-close-reward}
There is a constant $G$, depending only on $S$ and ${\left\Vert b \right\Vert}_{2}$, such that
for every $x \in X$ and $y \in D$,
$r \left( x , y \right) \ge v^{\star} \left( x \right) - G \operatorname{dist} \left( w \left( x , y \right) , \mathcal{L} \right)$.
\end{lemma}

\begin{lemma}\label{lem:sketch-few-updates}
Algorithm~\ref{alg:hard-ellipsoid-warmup} updates $V$ at most $O \left( \log n  \right)$ times.
\end{lemma}

Lemma~\ref{lem:sketch-close-reward} is a straightforward consequence of the fact that $r$ is affine. Lemmas~\ref{lem:sketch-ellipsoid-close} and~\ref{lem:sketch-few-updates} are due to the specific way we update $V$. Adding $\overline{w} \, \overline{w}^{T}$ to $V$ has the effect that it expands $E_{V}$ in the correct direction. {This ensures that the distance between $E_{V}$ and the true $\mathcal{L}$ grows only by the noise in $\overline{w}$, which is about $\frac{1}{\sqrt{n}}$.} Also, one can show that each update increases the volume of $E_{V}$ by a constant factor, which explains why we can have at most a logarithmic number of updates.
...
facts, together with {Lemmas~\ref{lem:sketch-ellipsoid-close} and~\ref{lem:sketch-close-reward}},
...
Finally, by Lemma~\ref{lem:sketch-few-updates} there are at most $O(\log n)$ informative and $O\left( \frac{T}{n}\right)$
uninformative blocks, so the total regret is about {$n \operatorname{log} n + \sqrt{n} \frac{T}{n}$}.

Ignoring logarithmic factors, this bound is minimized by choosing
$n = \left\lceil T^{2/3} \right\rceil$, giving an upper bound of
$\tilde{O} \left( T^{2/3} \right)$ on the regret.
\end{proof}
\endgroup
...
\draft{
Both problems contain a minimum over $y$ inside a maximum over $x$. The
second maximizes $H_V(x)$, which is itself a minimum over $y$. The first
asks whether some arm $x$ has $\hat K_V(x)=\varnothing$, that is, whether
every $y\in D$ has $w(x,y)$ outside $E_V$, and deciding this also means
minimizing over $y$ for each $x$. Theorem~\ref{thm:bienstock} handles a
single maximization, so we first need to convert each maximum of a
minimum into a maximum. The following lemma does this by writing the
inner minimum over $y$ in closed form, as a maximum over a new vector
$z=(z_0,z_X,z_D)\in\mathbb{R}^q$ whose coordinates match those of
$w(x,y)=(1,x,y)$.

\begin{lemma}\label{lem:hard-dual-forms}
Fix $x\in X$ and write $z=(z_0,z_X,z_D)\in\mathbb{R}^q$.
\begin{enumerate}
\item[(a)] $\hat K_V(x)=\varnothing$ if and only if
\begin{equation*}
\max_{\|z\|_2\le1}\left[z_0+z_X^Tx-\sqrt{z^TVz}-\|z_D\|_2\right]>0 .
\end{equation*}
\item[(b)] If $\hat K_V(x)\ne\varnothing$, then
\begin{equation*}
H_V(x)=a^Tx+c+\sup_{z\in\mathbb{R}^q}\left[z_0+z_X^Tx-\sqrt{z^TVz}-\|b+z_D\|_2\right].
\end{equation*}
\end{enumerate}
\end{lemma}

\begin{proof}
We use two facts. First, since $D$ is the unit ball, for every $z$,
\begin{equation*}
\min_{y\in D} z^Tw(x,y)=z_0+z_X^Tx+\min_{\|y\|_2\le1}z_D^Ty=z_0+z_X^Tx-\|z_D\|_2 .
\end{equation*}
Second, the largest value of $z^Tw$ over the ellipsoid $E_V$ is
\begin{equation*}
\max_{w\in E_V}z^Tw=\sqrt{z^TVz} ,
\end{equation*}
because writing $w=V^{1/2}u$ with $\|u\|_2\le1$ gives
$z^Tw=(V^{1/2}z)^Tu\le\|V^{1/2}z\|_2$, with equality at
$u=V^{1/2}z/\|V^{1/2}z\|_2$ when $z\ne0$; both sides vanish when $z=0$.

For (a), the set $\hat K_V(x)$ is empty exactly when the sets
$S_x\coloneqq\{w(x,y):y\in D\}$ and $E_V$ are disjoint. Both are compact
and convex. If they are disjoint, then a hyperplane strictly separates
them: there is a unit vector $z$ with
$\min_{w\in S_x}z^Tw>\max_{w\in E_V}z^Tw$. If they share a point $w$,
then $\min_{w'\in S_x}z^Tw'\le z^Tw\le\max_{w'\in E_V}z^Tw'$ for every
$z$. By the two facts, the difference
$\min_{w\in S_x}z^Tw-\max_{w\in E_V}z^Tw$ is the bracket in (a). The
bracket is continuous in $z$, so its maximum over the unit ball is
attained, and it is positive if and only if the sets are disjoint.

For (b), let $\iota(w)$ be $0$ if $w\in E_V$ and $+\infty$ otherwise, so
that $H_V(x)=\min_{y\in D}\left[r(x,y)+\iota(w(x,y))\right]$. We claim
that
\begin{equation*}
\iota(w)=\sup_{z\in\mathbb{R}^q}\left[z^Tw-\sqrt{z^TVz}\right] .
\end{equation*}
If $w\in E_V$, then $z^Tw\le\sqrt{z^TVz}$ for every $z$ by the second
fact, so every bracket is at most $0$, and $z=0$ gives $0$. If
$w\notin E_V$, then a hyperplane strictly separates $w$ from $E_V$: there
is a $z$ with $z^Tw>\sqrt{z^TVz}$. Replacing $z$ by $tz$ with $t>0$
multiplies the bracket by $t$, so letting $t\to\infty$ shows that the
supremum is $+\infty$. This proves the claim.
Substituting it into $H_V(x)$ gives
\begin{equation*}
H_V(x)=\min_{y\in D}\ \sup_{z\in\mathbb{R}^q}\ \phi(y,z),
\qquad
\phi(y,z)\coloneqq r(x,y)+z^Tw(x,y)-\sqrt{z^TVz} .
\end{equation*}
Here $\phi$ is affine in $y$, because $r(x,y)$ and
$z^Tw(x,y)=z_0+z_X^Tx+z_D^Ty$ are, and concave in $z$, because
$\sqrt{z^TVz}$ is a norm. Since $D$ is compact and convex, Sion's minimax
theorem~\citep{sion1958minimax} lets us exchange the minimum and the
supremum:
\begin{equation*}
H_V(x)=\sup_{z\in\mathbb{R}^q}\ \min_{y\in D}\ \phi(y,z) .
\end{equation*}
For fixed $z$, the terms of $\phi$ that involve $y$ are $b^Ty+z_D^Ty$,
so the first fact, applied to $b+z_D$ in place of $z_D$, gives
$\min_{y\in D}\phi(y,z)=a^Tx+c+z_0+z_X^Tx-\sqrt{z^TVz}-\|b+z_D\|_2$.
This proves (b).
\end{proof}

With the lemma, both problems become a single maximization over $x$ and
$z$ jointly. For the first problem, maximize
$z_0+z_X^Tx-\sqrt{z^TVz}-\|z_D\|_2$ over $x\in X$ and $\|z\|_2\le1$. By
part (a), some arm has an empty guessed outcome set if and only if the
maximum is positive, and in that case the $x$-component of a maximizer
is such an arm. For the second problem, take the supremum of
$a^Tx+c+z_0+z_X^Tx-\sqrt{z^TVz}-\|b+z_D\|_2$ over $x\in X$ and
$z\in\mathbb{R}^q$. By part (b), this supremum equals
$\max_{x\in X}H_V(x)$, and if the objective at some $(x,z)$ is within
$\varepsilon$ of the supremum, then $H_V(x)\ge\max_{x'\in X}H_V(x')-\varepsilon$.
Both objectives contain the product $z_X^Tx$, so neither problem is
convex. However, after replacing the two square roots by variables
$t,s\ge0$ with $z^TVz\le t^2$ and $\|z_D\|_2^2\le s^2$ (respectively
$\|b+z_D\|_2^2\le s^2$), each becomes a quadratic objective with a fixed
number of quadratic constraints. This is the form required by
Theorem~\ref{thm:bienstock}, except that the theorem also needs the
feasible region to be bounded, whereas $t$ and $s$ are only bounded from
below and, in the second problem, $z$ ranges over all of $\mathbb{R}^q$.

Of course, Theorem~\ref{thm:bienstock} also only gives us an approximate
solution. Appendix~\ref{app:hard-computation} handles both issues: it
bounds the variables, and it shows that one can choose the $\varepsilon$
as an appropriate function of $T$ such that the approximate solution is
still good enough that it does not change the regret rate. Since the
runtime depends logarithmically on $1/\varepsilon$, this only costs us
an extra $\log T$ factor in the runtime.
}
...
\begin{equation*}
U_{V , \lambda} \left( x \right) \coloneqq \operatorname*{min}_{y \in D} \left[ r \left( x , y \right) + \lambda p_{V} \left( x , y \right) \right] .
\end{equation*}


We pick the arm $x$ by maximizing $U_{V,\lambda}$. For now, assume that we can
...
\begin{lemma}\label{lem:soft-outcome-regret}
  ...
  every $\lambda>0$, and every arm $x$ maximizing $U_{V,\lambda}$,
...
\begin{proof}
We first claim that, with probability at least $1-\delta$, for every matrix
$V$ that Algorithm~\ref{alg:soft-square-root} considers, every arm $x'\in X$,
and every outcome $y\in D$,

\begin{equation*}
v^\star(x')-r(x',y)\le c\sqrt{p_V(x',y)},
\end{equation*}

where $c$ does not depend on $x'$, $y$, or $\lambda$; it is polynomial in
the dimensions and logarithmic in $T/\delta$, so $c=\tilde{O}(1)$. We prove
this in Appendix~\ref{app:soft-square-root-analysis}, as
Equation~\ref{eq:soft-reward-certificate}.

Now fix any arm $x'$ and outcome $y\in D$. Rearranging the claim and adding
the penalty $\lambda p_V(x',y)$ to both sides,

\begin{equation*}
r(x',y)+\lambda p_V(x',y)\ge v^\star(x')+\lambda p_V(x',y)-c\sqrt{p_V(x',y)}.
\end{equation*}

By the AM--GM inequality,
$\lambda p+\frac{c^2}{4\lambda}\ge2\sqrt{\lambda p\cdot\frac{c^2}{4\lambda}}=c\sqrt{p}$
for every $p\ge0$, so the right-hand side is at least
$v^\star(x')-\frac{c^2}{4\lambda}$. Minimizing over $y$ gives
$U_{V,\lambda}(x')\ge v^\star(x')-\frac{c^2}{4\lambda}$, and since $x$
maximizes the score, $U_{V,\lambda}(x)\ge V^\star-\frac{c^2}{4\lambda}$.
Finally, the definition of the score gives
$U_{V,\lambda}(x)\le r(x,y)+\lambda p_V(x,y)$ for every $y\in D$. Combining
the last two bounds,
$V^\star-r(x,y)\le\lambda p_V(x,y)+\frac{c^2}{4\lambda}$, which is the claim
with $\tilde{O}(1/\lambda)=\frac{c^2}{4\lambda}$. Since $c$ does not depend on
$\lambda$, the argument holds for all $\lambda>0$ on the same event.
\end{proof}
...
\caption{}\label{alg:soft-square-root}
\KwIn{Horizon $T$ and penalty parameter $\lambda>0$.}
$V\leftarrow I_q$,\quad $t\leftarrow0$\;
\While{$t<T$}{
  Define $U_{V,\lambda}(x)\coloneqq\min_{y\in D}[r(x,y)+\lambda p_V(x,y)]$ for every $x\in X$\;
  Choose $x\in\operatorname*{argmax}_{x'\in X}U_{V,\lambda}(x')$\;
  $n\leftarrow0$\;
  \Repeat{$n p_V(x,\overline{y}_n)\ge1$ or $t=T$}{
    Play $x$ and observe $y_{n+1}$\;
    $n\leftarrow n+1$,\quad $t\leftarrow t+1$\;
    $\overline{y}_n\leftarrow\frac{1}{n}\sum_{i=1}^n y_i$\;
  }
  \If{$n p_V(x,\overline{y}_n)\ge1$}{
    $\overline{w}_n\leftarrow w(x,\overline{y}_n)$\;
    $V\leftarrow V+n\overline{w}_n\overline{w}_n^{T}$\;
  }
}
\end{algorithm2e}
% \FloatBarrier
...
\begin{theorem}[Informal]\label{thm:soft-square-root}
  ...
  has expected regret $R_T \le \tilde{O}\left(\sqrt{T}\right)$.
\end{theorem}
\begingroup
\renewcommand{\proofname}{Proof sketch}
\begin{proof}
...
In this section we describe a polytime implementation of Algorithm~\ref{alg:soft-square-root}.

As in Section~\ref{sec:hard-ellipsoid-warmup}, the matrix updates and the stopping test can be computed
in polynomial time, and the main issue is choosing an arm. This now requires
solving a single problem: find an arm
$x\in\operatorname*{argmax}_{x'\in X}U_{V,\lambda}(x')$. The learner of
Section~\ref{sec:hard-ellipsoid-warmup} also had to
decide whether some arm has an empty guessed outcome set, because the hard
constraint $p_V(x,y)\le1$ can exclude every $y\in D$. Here the penalty
$\lambda p_V(x,y)$ is finite for every $y\in D$, so $U_{V,\lambda}(x)$ is
defined for every arm and no such test is needed.

The score is a minimum over $y$, so maximizing it over $x$ is not directly
a problem of the form in Theorem~\ref{thm:bienstock}. To fix this, we rewrite
the penalty as a maximum,
$\lambda w^{T}V^{-1}w=\max_{z}\left[z^{T}w-\frac{z^{T}Vz}{4\lambda}\right]$,
whose maximizer is $z=2\lambda V^{-1}w$. Exchanging the minimum over $y$
with this maximum, and writing $z=(z_0,z_X,z_D)$ to match the coordinates of
$w(x,y)=(1,x,y)$, gives

\begin{equation*}
U_{V,\lambda}(x)=a^{T}x+c+\max_{\|z\|_2\le4\lambda}
\left[z_0+z_X^{T}x-\|b+z_D\|_2-\frac{z^{T}Vz}{4\lambda}\right].
\end{equation*}

We maximize this expression jointly over $x\in X$ and $z$. Replacing the
norm $\|b+z_D\|_2$ by a variable $s\ge0$ with $\|b+z_D\|_2^2\le s^2$ and
adding a redundant ball constraint gives a quadratic objective with a fixed
number of quadratic constraints, one of which is a bounded ellipsoid, so
Theorem~\ref{thm:bienstock} applies.

Of course, this only gives us an approximate maximizer. Allowing an additive
error $\varepsilon>0$ when maximizing $U_{V,\lambda}$ adds at most
$n\varepsilon$ to the regret of a block of length $n$, and at most
$T\varepsilon$ over all rounds. Taking $\varepsilon=(T+1)^{-2}$ therefore
preserves the $\tilde{O}(\sqrt{T})$ regret bound, and since the runtime
depends logarithmically on $1/\varepsilon$, this only costs us an extra
$\log T$ factor in the runtime. Repairing the solver's slightly infeasible
output and keeping all numbers rational with polynomial bit length work
exactly as in Appendix~\ref{app:hard-computation}; the details are in
Appendix~\ref{app:soft-square-root-analysis}.
...
\section{Regret analysis of the $T^{2/3}$ learner}\label{app:hard-ellipsoid-analysis}
...
\begin{equation*}
\begin{aligned}
J_{\text{\normalfont max}} & \coloneqq \left\lceil q \left( 4 + 2 \operatorname{log}_{2} n \right) \right\rceil , \\
...
\end{equation*}
  ...
  $C_{r} T \delta$. Taking $n = \left\lceil T^{2/3} \right\rceil$ and
  $\delta = {\left( T + 1 \right)}^{- 2}$ give $R_{T} \le \tilde{O} \left( T^{2/3} \right)$ for fixed problem
  parameters.
...
\draft{Each update adds at most three to the trace, since ${\left\Vert \overline{w} \right\Vert}_{2}^{2} \le 3$,
so after $J$ updates $\operatorname{tr} V \le \frac{q}{n} + 3 J$. The initial determinant
is $n^{- q}$. Using the arithmetic--geometric mean inequality on the
eigenvalues, we get


\begin{equation*}
2^{J} \le \frac{\operatorname{det} V}{\operatorname{det} \left( n^{- 1} I_{q} \right)} \le {\left( 1 + 3 \frac{n J}{q} \right)}^{q} .
\end{equation*}


Write $\xi \coloneqq \frac{J}{q}$, so that $\xi \le \operatorname{log}_{2} \left( 1 + 3 n \xi \right)$. If $\xi \ge 4$, then
$\operatorname{log}_{2} \left( 1 + 3 n \xi \right) \le \operatorname{log}_{2} \left( 4 n \xi \right) = 2 + \operatorname{log}_{2} n + \operatorname{log}_{2} \xi \le 2 + \operatorname{log}_{2} n + \frac{\xi}{2}$,
using $\operatorname{log}_{2} \xi \le \frac{\xi}{2}$ for $\xi \ge 4$, and hence $\xi \le 4 + 2 \operatorname{log}_{2} n$. In either case
$J \le q \left( 4 + 2 \operatorname{log}_{2} n \right) \le J_{\text{\normalfont max}}$. Note that the horizon $T$ does not enter:
the count is the logarithm of the ratio between the final volume of $E_{V}$, which is bounded
in terms of $J$ alone, and its initial volume, which is set by the block length $n$.}
...
For the analysis, fix $\delta \in \left( 0 , 1 \right)$, use $h_{T}$ and $G$ from
Appendix~\ref{app:hard-ellipsoid-analysis}, \draft{and define


\begin{equation*}
J_{\text{\normalfont max}}^{\prime} \coloneqq \left\lceil q \operatorname{log}_{2} \left( 1 + 3 \frac{T}{q} \right) \right\rceil , \quad
{B_{T}^{\prime}}^{2} \coloneqq 1 + J_{\text{\normalfont max}}^{\prime} h_{T}^{2} .
\end{equation*}


These play the roles of $J_{\text{\normalfont max}}$ and $B_{T}$ in Appendix~\ref{app:hard-ellipsoid-analysis}.
The horizon $T$ takes the place of the block length $n$ because this learner starts from
$V = I_{q}$ and weights each block by its length, so the trace of $V$ can grow to $q + 3 T$.}
...
\begin{theorem}\label{thm:soft-square-root-regret}
  ...
  each planning call maximizes $U_{V,\lambda}$ to within $\varepsilon\ge0$.
  Run in exact arithmetic, it satisfies with probability at least $1-\delta$,


\begin{equation*}
T V^{\star} - \sum_{t = 1}^{T} r \left( x_{t} , y_{t} \right) \le 4 \lambda \left( J_{\text{\normalfont max}}^{\prime} + 1 \right) + \frac{G^{2} {B_{T}^{\prime}}^{2} T}{4 \lambda} + T \varepsilon ,
\end{equation*}
...
\end{equation*}
  ...
  $J_{\text{\normalfont max}}^{\prime} , h_{T} , B_{T}^{\prime} , G$.
...
\begin{equation*}
u^{T} V u \le 1 + \sum_{j = 1}^{J} n_{j} \left( \frac{h_{T}^{2}}{n_{j}} \right) = 1 + J h_{T}^{2} \le {B_{T}^{\prime}}^{2} .
\end{equation*}
...
\begin{equation}
\operatorname{dist} \left( w , \mathcal{L} \right) \le B_{T}^{\prime} {\left\Vert w \right\Vert}_{V^{- 1}} .\label{eq:soft-subspace-certificate}
\end{equation}
...
\begin{equation}
v^{\star} \left( x \right) \le r \left( x , y \right) + G B_{T}^{\prime} \sqrt{p_{V} \left( x , y \right)} .\label{eq:soft-reward-certificate}
\end{equation}
...
\begin{equation*}
r \left( x , y \right) + \lambda p_{V} \left( x , y \right) \ge v^{\star} \left( x \right) + \lambda u^{2} - G B_{T}^{\prime} u \ge v^{\star} \left( x \right) - \frac{G^{2} {B_{T}^{\prime}}^{2}}{4 \lambda} .
\end{equation*}
...
\begin{equation}
U_{V , \lambda} \left( x \right) \ge v^{\star} \left( x \right) - \zeta_{T} , \quad \zeta_{T} \coloneqq \frac{G^{2} {B_{T}^{\prime}}^{2}}{4 \lambda} .\label{eq:soft-optimism}
\end{equation}



We choose an arm whose score is within $\varepsilon$ of the highest score,
so $U_{V , \lambda} \left( x \right) \ge V^{\star} - \zeta_{T} - \varepsilon$. The block's empirical mean is in
...
Adding this over at most $J_{\text{\normalfont max}}^{\prime} + 1$ blocks proves the high-probability
bound. If the noise bound fails, regret is still at most $C_{r} T$,
so taking expectations adds at most $C_{r} T \delta$.

The difference from the learner of Section~\ref{sec:hard-ellipsoid-warmup} is that we now pay at most $4 \lambda$
...
balance are therefore $\lambda J_{\text{\normalfont max}}^{\prime}$ and $\frac{T}{\lambda}$, giving the
square-root rate.
...
\begin{equation}
U_{V , \lambda} \left( x \right) = a^{T} x + c + \operatorname*{max}_{{\left\Vert z \right\Vert}_{2} \le 4 \lambda} \left[ z_{0} + z_{X}^{T} x - {\left\Vert b + z_{D} \right\Vert}_{2} - \frac{z^{T} V z}{4 \lambda} \right] .\label{eq:soft-conjugate-planner}
\end{equation}




We can now maximize $U_{V , \lambda}$ by solving the QCQP
...
$U_{V , \lambda} \left( x^{\prime} \right)$. The number of precision bits we need is polynomial in
...
toward zero keeps these points in the balls. The function $U_{V , \lambda}$ is
...
\draft{
rounds. Whenever the learner plays $x_{t}$, let the adversary use the point mass at
...
\draft{
Taking expectations and using the definition of $R_{T}$ for this adversary policy,
}
...
that we can divide by $x_{0}$. We can therefore simulate the adversary by returning
...
$S \ge \frac{\sqrt{55}}{8}$. To minimize the reward, the adversary chooses the smallest
...
The reward lies in $\left[ 0 , 1 \right]$ on $X \times D$. For an arm $x^{T}$, the adversary

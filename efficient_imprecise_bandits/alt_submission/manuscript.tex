...
in polynomial time. The main issue is choosing an arm, since it involves solving a min-max optimization. To choose an arm, first we need to decide whether there exists an arm $x$ such that $\hat{K}_V(x)$ is empty. Exact emptiness tests can be unstable when the two constraints just
...
the block can be cut short more quickly. However, the current arm score
...
\begin{lemma}\label{lem:soft-outcome-regret}
  ...
  for every matrix $V$ at the start of a block of Algorithm~\ref{alg:soft-square-root},

...
Here $B_{z}$, $C_{z}$, and $d_{z}$ are matrices and vectors of fixed
...
This dependence on $T$ is tight up to logarithmic factors, because linear
imprecise bandits contain stochastic linear bandits as a special case. In a
...
same regret. But stochastic linear bandits have a worst-case regret lower bound
of $\Omega \left( \sqrt{T} \right)$~\citep{dani2008stochastic}, and therefore so do linear imprecise
bandits.
...
\While{$t<T$}{
  Define $U_V(x)\coloneqq\min_{y\in D}[r(x,y)+\lambda p_V(x,y)]$ for every $x\in X$\;
  Choose $x\in\operatorname*{argmax}_{x'\in X}U_V(x')$\;
  $n\leftarrow0$\;

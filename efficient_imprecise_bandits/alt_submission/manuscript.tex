% Mechanically translated from the unchanged Typst manuscript.
...
This is the transversality condition used in \citep{kosoy2025imprecise}.
}
...
for the data defining $\mathcal{M}$, and let $| \mathcal{M} |$ denote the length of its
...
$z \mapsto \left( B_{z} , C_{z} , d_{z} \right)$. We restrict attention to succinctly represented
instances, for which $| \mathcal{M} | \le \operatorname{poly} \left( d_{X} , d_{D} , d_{Z} \right)$.
}

Now we state our main result.

\draft{
\begin{sourcetheorem}[efficient $\sqrt{T}$ learning]\label{thm:efficient-upper-bound}
...
\end{equation*}
  ...
  Theorem~\ref{thm:soft-square-root}.
...
\begin{sourcealgorithm}\label{alg:hard-ellipsoid-warmup}
...
\begin{sourcetheorem}[Informal]\label{thm:hard-ellipsoid-warmup}
...
{
\textbf{Proof sketch.} We will show that, with high probability, throughout the
...
$\tilde{O} \left( T^{\frac{2}{3}} \right)$ on the regret. $\blacksquare$
...
The matrix updates and the test $p_{V} \left( x , \overline{y} \right) > 1$ can be computed
...
to finite precision while retaining the $\tilde{O} \left( T^{\frac{2}{3}} \right)$ regret bound.
...
\begin{equation*}
{\hat{K}}_{V}^{\text{\normalfont in}} \left( x \right) \coloneqq \left\{ y \in D : p_{V} \left( x , y \right) \le 4 \right\} , \quad {\hat{K}}_{V}^{\text{\normalfont out}} \left( x \right) \coloneqq \left\{ y \in D : p_{V} \left( x , y \right) \le 9 \right\} .
\end{equation*}


It looks for an arm whose inner set is empty, and otherwise plans
against the outer sets. The update rule still uses $p_{V} > 1$. Thus we
only change the arm-selection rule by constant factors in the
ellipsoid's radius.

To find an empty inner set, let $d_{V} \left( x \right)$ be the Euclidean distance
between $2 E_{V}$ and $\{ \left( 1 , x , y \right) : y \in D \}$. Writing $z = \left( z_{0} , z_{X} , z_{D} \right)$,
the support functions of the ellipsoid and the unit ball give


\begin{equation*}
d_{V} \left( x \right) = \operatorname*{max}_{{\left\Vert z \right\Vert}_{2} \le 1} \left[ z_{0} + z_{X}^{T} x - 2 \sqrt{z^{T} V z} - {\left\Vert z_{D} \right\Vert}_{2} \right] .
\end{equation*}


In particular, $d_{V} \left( x \right) > 0$ certifies that the inner set is empty.
Maximize this expression jointly over $x \in X$ and $z$, to additive
accuracy $\eta \coloneqq \frac{1}{10 n}$. If the returned lower bound exceeds $\eta$,
play the corresponding arm. Otherwise $d_{V} \left( x \right) \le 2 \eta$ for every arm.
The gap between $2 E_{V}$ and $3 E_{V}$ then guarantees that every outer set
...
where $M \coloneqq 4 n \left( 1 + {\left\Vert b \right\Vert}_{1} \right)$ suffices. The strict margin obtained above
...
$z^{T} V z \le t^{2}$ and ${\left\Vert b + z_{D} \right\Vert}_{2}^{2} \le s^{2}$. The objective becomes
quadratic in $\left( x , z , t , s \right)$. All variables have explicit bounds, so we
can also add a redundant enclosing ball. Bienstock's theorem
\citep{bienstock2016cdt}, stated in Appendix~\ref{app:qcqp}, solves these problems
...
Appendix~\ref{app:hard-computation}.
...
set. Points in $3 E_{V}$ are at most three times as far from $\mathcal{L}$
as the earlier bound for $E_{V}$. Thus the two types of blocks still
cost $\tilde{O} \left( n \right)$ and $\tilde{O} \left( \sqrt{n} \right)$, respectively. Maximizing
$H_{V}^{\text{\normalfont out}}$ to additive accuracy $\varepsilon$ adds at most $T \varepsilon$
to the regret. Taking $\varepsilon = {\left( T + 1 \right)}^{- 2}$, and rounding the observations
to the precision specified in the appendix, gives a polynomial-time
implementation with the same $\tilde{O} \left( T^{\frac{2}{3}} \right)$ rate.
...
depended on two things: (a) the set of outcomes considered in $H_{V}$ to
...
$w \left( x , \overline{y} \right)$ lies inside or outside the ellipsoid $E_{V}$. Ideally,
if the observations collected so far have an empirical mean
$\overline{y}$ with $w \left( x , \overline{y} \right)$ already far outside $E_{V}$, that
...
evaluate throughout the block, even when $w \left( x , \overline{y} \right)$ lies outside
$E_{V}$. We obtain this by replacing the hard constraint $p_{V} \left( x , y \right) \le 1$
in $H_{V}$ with a penalty. For a parameter $\lambda > 0$, which we will
...
We pick the arm $x$ by maximizing $U_{V}$. By definition, every outcome
$y \in D$ satisfies $U_{V} \left( x \right) - r \left( x , y \right) \le \lambda p_{V} \left( x , y \right)$. This is what
...
observation. Let ${\overline{y}}_{n}$ be the mean of the first $n$ outcomes
in the block. Since the reward is affine, the reward accumulated so far
is $n r \left( x , {\overline{y}}_{n} \right)$. Applying the inequality above to this mean
gives


\begin{equation*}
n U_{V} \left( x \right) - \sum_{i = 1}^{n} r \left( x , y_{i} \right) \le \lambda n p_{V} \left( x , {\overline{y}}_{n} \right) .
\end{equation*}
...
how far our accumulated reward falls below $n U_{V} \left( x \right)$. We end the
block and update $V$ the first time this bound reaches $\lambda$, or
equivalently, when $n p_{V} \left( x , {\overline{y}}_{n} \right) \ge 1$. Before this happens,
the accumulated reward falls short of $n U_{V} \left( x \right)$ by less than
$\lambda$.
...
$\lambda \coloneqq \left\lceil \sqrt{T} \right\rceil$.


\begin{sourcealgorithm}\label{alg:soft-square-root}
...
\end{equation*}
    ...
    $U_{V} \left( x \right) \ge \operatorname*{max}_{x^{\prime} \in X} U_{V} \left( x^{\prime} \right) - \varepsilon$.
...
$I_{q} + \sum_{j} n {\overline{w}}_{j} {\overline{w}}_{j}^{T}$. We now allow the block
...
\begin{sourcetheorem}\label{thm:soft-square-root}

  Suppose nature follows any compatible adaptive policy. Then
  Algorithm~\ref{alg:soft-square-root}, run in exact arithmetic, satisfies with
...
\end{equation*}
  ...
  where each planning call maximizes $U_{V}$ to within $\varepsilon$. We can
  implement the algorithm in time polynomial in $\left| \mathcal{M} \right|$ and $T$
  under the weak finite-precision model. Taking
  $\varepsilon = \delta = {\left( T + 1 \right)}^{- 2}$ gives expected regret
...
\textbf{Proof.} As before, each completed block at least doubles the
...
weak-optimization theorem \citep{bienstock2016cdt} as described in Section~\ref{app:qcqp}.
...
polynomial time. This proves the computational claim. $\blacksquare$
}
...
\begin{sourcelemma}\label{lem:planning-from-learning}
...
\draft{
\textbf{Proof.} Fix $z \in Z$ and $\varepsilon \in \left( 0 , 1 \right)$, set $\eta \coloneqq \frac{\varepsilon}{6}$, and
...
because $\beta$ is fixed. $\blacksquare$
}
...
\textbf{Proof.} We reduce from MAX-CUT, as defined in Appendix~\ref{app:max-cut}. Let $\left( G , k \right)$ be an
...
\begin{sourcelemma}\label{lem:simplex-cut-norm}
...
\textbf{Proof.} Fix $\sigma \in {\left\{ - 1 , 1 \right\}}^{n}$. For every edge $e = \left\{ i , j \right\}$,
...
Summing over $e \in E$ and taking the square root proves the identity. $\blacksquare$
...
\begin{sourcelemma}\label{lem:simplex-duality}
...
\draft{
\textbf{Proof.} Choosing the sign of each coordinate gives
}
...
\draft{
\textbf{Proof.} Each edge coordinate $x_{e}$ contributes $x_{e}$ and $- x_{e}$ to two
...
${\left\Vert x \right\Vert}_{2} \le 1$, and the last uses $n \ge 2$, which follows from $m \ge 1$. $\blacksquare$
}


\begin{sourcelemma}\label{lem:simplex-arm-value}
...
\draft{
\textbf{Proof.} Fix $x \in X$ and set $t \coloneqq \gamma B_{G}^{T} x$. The preceding lemma
...
every coordinate, so its reward is the minimum. $\blacksquare$
}
...
\begin{sourcetheorem}\label{thm:additive-polytope-np-hardness}
...
\draft{
\textbf{Proof.} We again reduce from MAX-CUT. Let $G = \left( V , E \right)$ have $n$ vertices and
...
\begin{sourcelemma}\label{lem:clique-cubic}
...
\draft{
\textbf{Proof.} Write $\rho \coloneqq {\left\Vert u \right\Vert}_{2}$ and $\sigma \coloneqq {\left\Vert w \right\Vert}_{2}$. For fixed $u$ and
...
maximum equals its maximum absolute value. $\blacksquare$
}


\begin{sourcetheorem}\label{thm:trilinear-balls-np-hardness}
...
\draft{
\textbf{Proof.} We may restrict CLIQUE to instances with $m \ge 1$ and
...
\begin{sourcetheorem}\label{thm:iucb-np-hardness}
...
\draft{
\textbf{Proof.} Given a rational order-three tensor
...
optimal first arm is NP-hard as well. $\blacksquare$
}
...
\begin{sourcetheorem}\label{thm:easy-planning-hard-learning}
...
\draft{
\textbf{Proof.} Fix $2 \le k \le n$, and let
}
...
\end{equation*}
  ...
  \citep{bienstock2016cdt}. The underlying weak-feasibility step reduces to a fixed
...
\item \draft{
  Second, the planning reduction in Section~\ref{sec:soft-square-root} has six
  ...
  $\varepsilon$ in time $\operatorname{poly} \left( L , \operatorname{log} \left( \frac{1}{\varepsilon} \right) \right)$. The same section explains
...
\begin{sourcelemma}\label{lem:subspace-reward-bound}
...
\textbf{Proof.} Let $N \coloneqq \operatorname{ker} C_{z^{\star}}$ and let $Q$ be the orthogonal projector
...
at most ${\left\Vert b \right\Vert}_{2}$ times this distance, which proves the claim. $\blacksquare$

We also need to know how close a block average is to the average of the
true conditional means.


\begin{sourcelemma}\label{lem:interval-noise}
...
\textbf{Proof.} Each coordinate of $y_{t} - m_{t}$ is a martingale difference
...
and summing the squared coordinate bounds give the claim. $\blacksquare$
...
\begin{sourcelemma}\label{lem:subspace-distance-bound}
...
\textbf{Proof.} Number the informative blocks $j = 1 , \ldots , J$. Write $x_{j}$ for
...
\begin{sourcetheorem}\label{thm:hard-ellipsoid-regret}
...
\textbf{Proof.} First, there cannot be very many informative blocks. Each one
...
adds at most $C_{r} T \delta$. $\blacksquare$


\subsection{Finite-precision implementation}\label{app:hard-computation}

We justify the implementation in Section~\ref{sec:hard-computation}. The matrix
always satisfies
...
The distance formula follows by writing distance to $2 E_{V}$ as
...
This point lies strictly inside $3 E_{V}$. We may therefore apply convex
...
$\left\lceil \frac{T}{n} \right\rceil$ planning calls, each taking time polynomial in the input
bit length, $T$, and $\operatorname{log} \left( \frac{1}{\varepsilon} \right)$. This proves the claimed
implementation and regret bound. $\blacksquare$
}
